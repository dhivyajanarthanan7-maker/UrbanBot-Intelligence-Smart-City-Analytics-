...
# ================= PAGE CONFIG =================
st.set_page_config(
    page_title="Air Quality AI Insights",
    layout="wide"
)
...
# ================= POLLUTANT SELECTION =================
st.markdown("---")
st.subheader("🧪 Pollutant Inputs (from dataset)")
...
for i, pol in enumerate(pollutants):
    pol_df = location_df[location_df["pollutant_id"].str.upper() == pol.upper()]
    if not pol_df.empty:
        options = sorted(pol_df["pollutant_avg"].dropna().unique())
        values[pol.upper()] = cols[i % 3].selectbox(
            pol.upper(),
            options
        )
    else:
